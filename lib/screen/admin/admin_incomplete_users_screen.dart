...
class _AdminIncompleteUsersScreenState
  ...
  bool _importingIncomplete = false;
  bool _sendingIncompleteEmails = false;
  bool _resettingSentEmails = false;
  ...
  Future<void> _syncEmailIndex() async {
    try {
      ...
      if (mounted) setState(() => _syncingEmails = false);
    }
  }

  Future<void> _importIncompleteUsers() async {
    try {
      setState(() => _importingIncomplete = true);

      final result = await FirebaseFunctions.instance
          .httpsCallable('importIncompleteUsers')
          .call();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: Text(
            ...
            'Aprobados omitidos: ${data?['skippedApproved'] ?? 0} | '
            'Ya existían: ${data?['skippedAlreadyExists'] ?? 0}',
          ),
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          backgroundColor: Colors.red,
          content: Text('Error importando incompletos: $e'),
        ),
      );
    } finally {
      if (mounted) setState(() => _importingIncomplete = false);
    }
  }

  Future<void> _resetSentEmailsToPending() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: _card,
        title: const Text(
          'Volver correos a pendiente',
          style: TextStyle(color: Colors.white),
        ),
        content: const Text(
          'Esto tomará los usuarios incompletos que ya tienen correo enviado y los dejará nuevamente como pendientes para poder reenviar la operación.',
    ...
    try {
      setState(() => _resettingSentEmails = true);

      final snap = await FirebaseFirestore.instance
          .collection('incomplete_users')
          .where('email_sent', isEqualTo: true)
          .get();
      ...
      if (snap.docs.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            ...
            content: Text('No hay correos enviados para volver a pendiente.'),
          ),
        );
        return;
      }
      ...
      for (final doc in snap.docs) {
        batch.set(
          doc.reference,
          {
            ...
            'email_reset_to_pending_at': FieldValue.serverTimestamp(),
          },
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          backgroundColor: Colors.orange,
          content: Text(
            'Correos enviados vueltos a pendiente: ${snap.docs.length}',
          ),
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          backgroundColor: Colors.red,
          content: Text('Error al volver correos a pendiente: $e'),
        ),
      );
    } finally {
      if (mounted) setState(() => _resettingSentEmails = false);
    }
  }

  Future<void> _sendIncompleteUsersEmails() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: _card,
        ...
        content: const Text(
          'Se enviará el correo "Completa tu cuenta" a usuarios incompletos pendientes. ¿Deseas continuar?',
          style: TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
    ...
    try {
      setState(() => _sendingIncompleteEmails = true);

      final result = await FirebaseFunctions.instance
          .httpsCallable('sendIncompleteUsersEmails')
          .call({
        'dryRun': false,
        'limit': 40,
      });
      ...
      if (mounted) setState(() => _sendingIncompleteEmails = false);
    }
  }
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final sentDates = docs
              .map((d) => d.data()['email_sent_at'])
              .whereType<Timestamp>()
              .toList();

          sentDates.sort((a, b) => b.compareTo(a));

          final lastEmailSentAt = sentDates.isNotEmpty
              ? _formatValue(sentDates.first)
              : 'Sin envíos';
            ...
            return ListView(
              ...
              children: [
                _HeaderCard(
                  ...
                  importingIncomplete: _importingIncomplete,
                  sendingIncompleteEmails: _sendingIncompleteEmails,
                  resettingSentEmails: _resettingSentEmails,
                  onSyncEmails: _syncEmailIndex,
                  onImportIncompleteUsers: _importIncompleteUsers,
                  onResetSentEmailsToPending: _resetSentEmailsToPending,
                  onSendIncompleteUsersEmails: _sendIncompleteUsersEmails,
                ),
                const _EmptyState(),
              ],
            );
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final reason = (data['reason'] ?? 'Sin motivo').toString();
              final source = (data['source'] ?? 'Sin origen').toString();
              final emailWasSent = data['email_sent'] == true;
              final createdAt = _formatValue(data['createdAt']);
              final lastSignIn = _formatValue(data['lastSignInTime']);
              final importedAt = _formatValue(data['imported_at']);
              final emailSentAt = _formatValue(data['email_sent_at']);

              return Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16),
                child: _IncompleteUserCard(
                  uid: uid,
                  email: email,
                  reason: reason,
                  source: source,
...
class _HeaderCard extends StatelessWidget {
  ...
  final bool importingIncomplete;
  final bool sendingIncompleteEmails;
  final bool resettingSentEmails;
  final VoidCallback onSyncEmails;
  final VoidCallback onImportIncompleteUsers;
  final VoidCallback onResetSentEmailsToPending;
  final VoidCallback onSendIncompleteUsersEmails;

  const _HeaderCard({
    required this.total,
    required this.emailSent,
    required this.pendingEmail,
    required this.lastEmailSentAt,
    required this.syncingEmails,
    required this.importingIncomplete,
    required this.sendingIncompleteEmails,
    required this.resettingSentEmails,
    required this.onSyncEmails,
    required this.onImportIncompleteUsers,
    required this.onResetSentEmailsToPending,
    required this.onSendIncompleteUsersEmails,
  });
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Correos enviados: $emailSent | Último envío: $lastEmailSentAt',
                    style: const TextStyle(
                      color: Colors.white,
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: importingIncomplete ? null : onImportIncompleteUsers,
              icon: importingIncomplete
                  ? const SizedBox(
                      width: 18,
                      height: 18,
                      child: CircularProgressIndicator(
              ...
              label: Text(
                importingIncomplete
                    ? 'Importando usuarios incompletos...'
                    : 'Importar usuarios incompletos',
              ),
              style: ElevatedButton.styleFrom(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed:
                  resettingSentEmails ? null : onResetSentEmailsToPending,
              icon: resettingSentEmails
                  ? const SizedBox(
                      width: 18,
                      height: 18,
                      child: CircularProgressIndicator(
              ...
              label: Text(
                resettingSentEmails
                    ? 'Volviendo enviados a pendiente...'
                    : 'Volver correos enviados a pendiente',
              ),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.orange,
                foregroundColor: Colors.white,
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: sendingIncompleteEmails
                  ? null
                  : onSendIncompleteUsersEmails,
              icon: sendingIncompleteEmails
                  ? const SizedBox(
                      width: 18,
                      height: 18,
                      child: CircularProgressIndicator(
              ...
              label: Text(
                sendingIncompleteEmails
                    ? 'Enviando correos...'
                    : 'Enviar correo: Completa tu cuenta',
              ),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.green,
                foregroundColor: Colors.white,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  if (emailSent)
                    _InfoLine(label: 'Correo enviado', value: emailSentAt),
                ],
              ),
            ),
