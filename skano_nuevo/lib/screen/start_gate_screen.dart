...
class _StartGateScreenState extends State<StartGateScreen> {
  ...
  Future<bool> _requiresForceUpdate() async {
    try {
      ...
      final minimumBuildRaw = data["minimum_build"];
      final minimumBuild = minimumBuildRaw is int
          ? minimumBuildRaw
          : int.tryParse(minimumBuildRaw.toString()) ?? 0;
  ...
  Future<void> _validateStartGate() async {
    try {
      ...
      // 0️⃣ Validar actualización obligatoria desde Firestore
      ...
      final identityChangePending =
          data["identity_change_pending"] == true;
      ...
      // 6️⃣ Registrar última actividad
      try {
  ...
  Widget build(BuildContext context) {
    return const Scaffold(
      backgroundColor: Color(0xFF07091F),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.security, size: 120, color: Color(0xFF0A6CFF)),
            SizedBox(height: 20),
            Text(
              "SKANO",
              style: TextStyle(
